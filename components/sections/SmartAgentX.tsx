  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 z-0">
        ...
        <motion.div 
          className="absolute w-[800px] h-[800px] bg-[#0F4C3A]/5 rounded-full blur-[150px] pointer-events-none"
      ...
      <div className="relative z-10 w-full max-w-7xl flex flex-col items-center">
        <div className="text-center mb-24">
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-[#0F4C3A]/10 border border-[#0F4C3A]/30 text-emerald-200 text-[12px] font-bold uppercase tracking-[0.4em] mb-12 font-mono"
          >
            <Cpu className="w-3 h-3" />
            {t("neural_interface")}
          </motion.div>
          <h2 className="text-5xl md:text-[8rem] font-black tracking-tighter text-white mb-10 leading-[0.8]">
             The power behind <span className="text-transparent bg-clip-text bg-gradient-to-r from-emerald-200 via-blue-500 to-[#0F4C3A] animate-gradient pb-4 block mt-4">SmartArt AI</span>
          </h2>
          ...
          >
            <button 
              ...
              className="px-12 py-5 rounded-full bg-[#0F4C3A] text-white font-black uppercase tracking-[0.3em] text-[11px] hover:scale-105 transition-all shadow-[0_0_40px_rgba(15,76,58,0.4)]"
            >
              {t("test_btn")}
            </button>
          </motion.div>
        ...
        <div className="relative w-full aspect-auto min-h-[800px] md:aspect-[21/9] md:min-h-0 max-w-7xl group">
          <div className="absolute -inset-1 bg-gradient-to-r from-[#0F4C3A] via-[#0F4C3A] to-[#0F4C3A] rounded-[3.5rem] opacity-20 group-hover:opacity-40 blur-2xl transition-all duration-1000"></div>
          ...
          <div className="absolute inset-0 bg-[#080808] backdrop-blur-[40px] rounded-[3.5rem] border border-white/10 flex flex-col md:overflow-hidden shadow-2xl">
            ...
            <div className="flex-1 w-full grid grid-cols-1 md:grid-cols-3 gap-12 md:gap-0 p-8 md:p-12 relative overflow-y-auto md:overflow-hidden">
              ...
              <div className="flex flex-col md:border-r border-white/5 md:pr-8">
                <div className="flex items-center gap-3 text-emerald-200 mb-8">
                  ...
                  <AnimatePresence mode="popLayout">
                    {logs.map((log, idx) => (
                      <motion.div
                        ...
                        exit={{ opacity: 0, x: 10 }}
                        className={`whitespace-nowrap flex items-center gap-2 ${idx === 0 ? 'text-emerald-200 font-bold' : 'text-white/50'}`}
                      >
                         ...
                         {idx === 0 && (
                            <motion.span 
                              ...
                              className="w-1.5 h-3 bg-[#0F4C3A] inline-block"
                            />
                         )}
                      </motion.div>
                    ))}
                        ...
                        <div key={i} className="flex items-center gap-4 group/row">
                           ...
                           <div className="w-2.5 h-2.5 rounded-full border border-white/10 flex items-center justify-center p-0.5">
                              <motion.div
                                animate={{
                                  scale: [1, 1.5, 1],
                                  backgroundColor: ["#ef4444", "#0F4C3A", "#ef4444"]
                                }}
                                transition={{ duration: 3, repeat: Infinity, delay: i * 0.5 }}
                                className="w-full h-full rounded-full"
                              />
                           </div>
                           <div className="flex-1 text-[11px] text-emerald-200 font-bold truncate">{item.a}</div>
                        </div>
                      ))}
                   </div>
                </div>
              ...
              <div className="flex flex-col md:border-l border-white/5 md:pl-8">
                ...
                <div className="grid grid-cols-2 gap-y-12">
                   {[
                     { label: "Svarstid", val: `${metrics.latency}ms`, color: "text-emerald-200", sub: "OPTIMAL" },
                     ...
                     { label: "Åtgärder", val: `${metrics.completed}`, color: "text-emerald-200", sub: "PROCESSED" }
                   ].map((m, i) => (
                   ...
                   <div className="flex gap-1">
                      {[1,2,3,4,5].map(i => (
                        <motion.div
                          ...
                          className="w-1 bg-[#0F4C3A]/20 rounded-t-sm"
                        />
                      ))}
                   </div>
                </div>
        ...
        >
          <div className="flex items-center gap-3">
            <div className="w-1.5 h-1.5 rounded-full bg-[#0F4C3A] shadow-[0_0_10px_#0F4C3A]" />
      ...
      <AnimatePresence>
        {showChallenge && (
          ...
          >
            ...
            >
              <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-emerald-200 via-[#0F4C3A] to-emerald-200" />
              ...
              {challengeStep === 1 && (
                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-8">
                  <div className="flex items-center gap-4 text-emerald-200">
                    <span className="w-10 h-10 rounded-xl bg-[#0F4C3A]/10 flex items-center justify-center font-mono font-black text-sm">01</span>
                  ...
                  <div className="space-y-4">
                    <input 
                      ...
                      className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-5 text-white focus:outline-none focus:border-[#0F4C3A] transition-all font-mono text-sm"
                    />
                    <button 
                      onClick={handleInference} disabled={!challengeData.companyInfo || isAnalyzing}
                      className="w-full py-5 rounded-2xl bg-[#0F4C3A] text-white font-black hover:bg-[#0F4C3A] transition-all font-mono text-xs tracking-[0.3em] uppercase flex items-center justify-center gap-3"
                    >
              ...
              {challengeStep === 2 && (
                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-8">
                  <div className="flex items-center gap-4 text-emerald-200">
                    <span className="w-10 h-10 rounded-xl bg-[#0F4C3A]/10 flex items-center justify-center font-mono font-black text-sm">02</span>
                  ...
                  <p className="text-white/40 font-mono text-sm leading-relaxed">
                    {t("step2_desc_prefix")} <span className="text-emerald-200 font-bold underline underline-offset-4">{challengeData.inferredProcess}</span>.
                  </p>
                  <div className="space-y-3 pt-4">
                    {[t("step2_opt1"), t("step2_opt2"), t("step2_opt3")].map((opt, i) => (
                      <button 
                        key={i} onClick={() => setChallengeStep(3)}
                        className="w-full p-5 rounded-2xl bg-white/[0.03] border border-white/5 text-left text-white/60 hover:bg-[#0F4C3A]/10 hover:border-[#0F4C3A]/50 hover:text-emerald-200 transition-all font-mono text-sm"
                      >
                        {opt}
                      </button>
                    ))}
              ...
              {challengeStep === 3 && (
                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-8">
                  <div className="flex items-center gap-4 text-emerald-200">
                    <span className="w-10 h-10 rounded-xl bg-[#0F4C3A]/10 flex items-center justify-center font-mono font-black text-sm">03</span>
                  ...
                  <div className="space-y-4">
                    ...
                    <button 
                      ...
                      className="w-full py-5 rounded-2xl bg-[#0F4C3A] text-white font-black hover:bg-[#0F4C3A] transition-all font-mono text-xs tracking-[0.3em] uppercase"
                    >
                      {t("map_btn")}
                    </button>
                  </div>
              ...
              {challengeStep === 4 && (
                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-8">
                  <div className="flex items-center gap-4 text-emerald-200">
                    <span className="w-10 h-10 rounded-xl bg-[#0F4C3A]/10 flex items-center justify-center font-mono font-black text-sm">04</span>
              ...
              {challengeStep === 5 && (
                <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className="space-y-8">
                  <div className="flex items-center gap-4 text-emerald-200">
                    <span className="w-10 h-10 rounded-xl bg-[#0F4C3A]/10 flex items-center justify-center font-mono font-black text-sm">05</span>
              ...
              {challengeStep === 6 && (
                <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="space-y-8">
                   <div className="p-6 rounded-2xl bg-[#0F4C3A]/5 border border-[#0F4C3A]/20 font-mono text-[11px] leading-relaxed text-white/70">
                      <div className="text-emerald-200 font-bold mb-4 flex items-center gap-2">
                         <div className="w-2 h-2 rounded-full bg-[#0F4C3A] shadow-[0_0_10px_#0F4C3A]" />
                      ...
                      try {
                        await fetch("/api/contact", {
                          ...
                          body: JSON.stringify({
                            ...
                            Foretag: challengeData.companyInfo,
                            ...
                            Tjanst: "AgentX Intelligence Challenge",
                  ...
                  >
                    <div className="grid grid-cols-2 gap-4">
                      <input 
                        ...
                        className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-4 text-white font-mono text-xs focus:border-[#0F4C3A] outline-none"
                      />
                      <input 
                    ...
                    <button 
                      ...
                      className="w-full py-5 rounded-2xl bg-[#0F4C3A] text-white font-black font-mono text-xs tracking-[0.3em] uppercase hover:bg-[#136049] transition-all"
                    >
