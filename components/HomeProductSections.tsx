...
function ProductRail({
  ...
  return (
    <section
      ...
      className="reveal-fade scroll-mt-8"
    >
      <div className="flex items-end justify-between gap-3">
        <div className="min-w-0">
          ...
          <p className="mt-1 max-w-[25rem] text-sm leading-6 text-ink-soft sm:max-w-[34rem] sm:text-base sm:leading-7">
            {subtitle}
          </p>
        </div>
        <Link
        ...
        <div className="-mx-4 mt-3 max-w-[100vw] overflow-x-auto px-4 pb-3 [scrollbar-width:none] sm:-mx-8 sm:px-8 lg:mx-0 lg:px-0 [&::-webkit-scrollbar]:hidden">
          <div className="flex w-max snap-x snap-mandatory gap-3 sm:gap-4 lg:grid lg:w-auto lg:grid-cols-3 lg:gap-6">
            {items.map((product, index) => (
              <div
                key={product.slug}
                className="reveal-fade w-[15.75rem] shrink-0 snap-start min-[390px]:w-[16.5rem] sm:w-[19rem] lg:w-auto"
...
function HomeProductCard({ product }: { product: Product }) {
  ...
  return (
    <article className="group h-full overflow-hidden rounded-[1.65rem] border border-line bg-surface/78 shadow-[0_20px_58px_rgba(216,131,146,0.13)] backdrop-blur-xl transition hover:-translate-y-1 hover:bg-surface sm:rounded-[2rem]">
      <div className="relative aspect-square overflow-hidden bg-[#f7e8e8] sm:aspect-[4/3]">
        ...
        <div className="absolute left-3 top-3 inline-flex items-center gap-1.5 rounded-full bg-wine/90 px-2.5 py-1 text-[0.68rem] font-black text-bg shadow-[0_12px_32px_rgba(109,60,77,0.24)] backdrop-blur sm:left-4 sm:top-4 sm:px-3 sm:text-xs">
        ...
        <div className="absolute right-3 top-3">
          <SaveProductButton
            ...
            className="grid size-9 place-items-center rounded-full border border-line bg-surface/90 text-wine shadow-[0_12px_32px_rgba(91,52,65,0.14)] backdrop-blur transition hover:-translate-y-0.5 hover:bg-surface sm:size-10"
          />
        </div>
      </div>
      ...
      <div className="p-4 sm:p-5">
        ...
        <p className="mt-3 text-[13px] leading-5 text-ink-soft sm:text-sm sm:leading-6">
          {product.summary}
        </p>
        <ProductBadges badges={product.badges.slice(0, 2)} className="mt-4" />

        <div className="mt-4 flex flex-wrap items-center gap-2 text-[0.68rem] font-black text-ink-soft sm:text-xs">
          {score ? (
            <ScoreBadge score={score} className="text-[0.68rem] sm:text-xs" />
          ) : null}
          <span className="rounded-full bg-surface/70 px-3 py-2">Annons</span>
        </div>

        <div className="mt-5 grid grid-cols-1 gap-2 min-[390px]:grid-cols-[1fr_auto]">
          <Link
            href={productHref}
            className="inline-flex min-h-10 items-center justify-center gap-2 rounded-full bg-wine px-4 text-xs font-black text-bg shadow-[0_16px_38px_rgba(109,60,77,0.3)] transition hover:-translate-y-0.5 hover:bg-wine/90 sm:min-h-11 sm:text-sm"
          >
            Elins koll
          ...
          <a
            ...
            className="inline-flex min-h-10 items-center justify-center rounded-full border border-rose/45 bg-surface/70 px-4 text-xs font-black text-wine transition hover:-translate-y-0.5 hover:bg-surface sm:min-h-11 sm:text-sm"
