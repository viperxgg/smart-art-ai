...
export default function FragaElinPage() {
  return (
    ...
    >
      ...
      <section className="relative mx-auto flex h-[100dvh] min-h-[40rem] w-full max-w-6xl flex-col px-4 pb-[env(safe-area-inset-bottom)] pt-4 sm:px-6 lg:h-auto lg:min-h-screen lg:px-8 lg:pb-[calc(8rem+env(safe-area-inset-bottom))] lg:pt-5">
        <header className="mb-3 shrink-0">
          <h1 className="font-display text-2xl leading-tight text-ink sm:text-3xl">Fråga Elin – valfri AI-hjälp</h1>
          <p className="mt-1 text-xs leading-5 text-ink-soft">Annons: sidan innehåller affiliatelänkar. AI-svar kan vara fel och ersätter inte personlig medicinsk rådgivning.</p>
          <Link href="/jamforelser" className="inline-flex min-h-11 items-center text-sm font-bold text-wine underline underline-offset-4">Läs jämförelserna utan att chatta</Link>
        </header>
        <div className="grid min-h-0 flex-1 gap-6 lg:grid-cols-[0.82fr_1.18fr] lg:items-stretch lg:py-10">
          <aside className="hidden flex-col justify-between rounded-[1.8rem] border border-line bg-surface/70 p-5 shadow-[0_26px_80px_rgba(216,120,141,0.12)] backdrop-blur-xl sm:p-7 lg:flex">
            <div>
              ...
              <h2 className="mt-5 font-display text-4xl leading-[1.04] tracking-normal text-ink sm:text-5xl lg:text-6xl">
