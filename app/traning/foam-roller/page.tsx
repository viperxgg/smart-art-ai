...
const pageUrl = `${siteConfig.url}/traning/foam-roller`;

const faqSchema = {
  "@context": "https://schema.org",
  "@type": "FAQPage",
  mainEntity: foamRollerFaqItems.map((item) => ({
    "@type": "Question",
    name: item.question,
    acceptedAnswer: {
      "@type": "Answer",
...
const breadcrumbItems = [
  ...
  { name: "Foam roller", href: "/traning/foam-roller" },
];

const breadcrumbSchema = buildBreadcrumbSchema(breadcrumbItems);

export const metadata = createSeoMetadata({
  title: "Foam roller - Elins guide till bästa valen 2026 | Elins val",
  description:
    "Elin jämför BODYMATE CARE som enkel slät foam roller med Elvire Deep Tissue som komplett texturerat återhämtningsset.",
  url: pageUrl,
  image: {
    url: `${siteConfig.url}${foamRollerPicks[0].product.image}`,
    width: 1200,
    height: 900,
    alt: foamRollerPicks[0].product.imageAlt,
  },
});

export default function FoamRollerHubPage() {
  return (
    <main
      id="content"
      ...
      <div className="mx-auto w-full max-w-5xl">
        ...
        <section className="mt-10 overflow-hidden rounded-[2.4rem] border border-line bg-surface/72 p-7 shadow-[0_30px_90px_rgba(185,131,166,0.12)] md:p-10">
          ...
          <h1 className="editorial-color-kiss mt-6 max-w-4xl font-display text-5xl leading-[1.02] tracking-[-0.04em] sm:text-7xl">
            Foam roller - enkel rulle eller helt set?
          </h1>
          <p className="mt-6 max-w-3xl text-xl leading-9 text-ink-soft">
            Elin har valt två tydliga spår: BODYMATE CARE om du vill ha en
            fokuserad, slät och fast rulle med ryggränna, och Elvire om du vill
            ha fler verktyg för fötter, triggerpunkter och djupare massage.
          </p>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="mt-4 max-w-3xl text-lg leading-8 text-ink-soft">
                Välj BODYMATE om du vill ha en enkel rulle som gör jobbet utan
                extra delar. Välj Elvire om du vill ha ett helt återhämtningskit
                med texturerad rulle, boll, stickrulle och fotmassör.
              </p>
            </div>
          </div>
        </section>
        ...
        <AmazonPurchaseLinks
          products={foamRollerPicks.map((pick) => pick.product)}
          className="mt-6"
        />
        ...
        <section className="reveal-fade mt-12 overflow-hidden rounded-[2rem] border border-line bg-surface/72 shadow-[0_24px_70px_rgba(185,131,166,0.1)]">
          ...
          <div className="overflow-x-auto">
            <table className="w-full min-w-[44rem] border-collapse text-left">
              ...
              <tbody>
                {foamRollerComparisonRows.map(([label, bodymate, elvire]) => (
